...
#                infoMeta = False
            ...
            if infoMeta == True:
                ...
                try:
                    self.name = urllib.quote_plus(title)
                except:
...
#        import xbmc
#        xbmc.log(str(imdb)+'===>REALIZER', level=xbmc.LOGINFO)
...
#            self.infolabels = {"imdb": imdb, "IMDBNumber": imdb, "MovieID": imdb}    
            ...
            if self.content == 'episode':
                ...
                item = xbmcgui.ListItem(meta['label'], thumb)
                item.setProperty('Episode', meta['episode'])
                item.setProperty('Season', meta['season'])
                item.setProperty('Content', 'episodes')
            if self.content != 'episode':
...
#        if self.content == 'movie':
#            self.infolabels = {"imdb": imdb, "IMDBNumber": imdb, "MovieID": imdb}    
            self.infolabels.update({"imdb": imdb, "IMDBNumber": imdb, "MovieID": imdb}    )

            if self.content == 'episode' and infoMeta == True:
                item.setProperty('TVShowTitle', meta['tvshowtitle'])
                self.infolabels.update({"Title": meta['title'], "path": url, "Season": meta['season'], "Episode": meta['episode'], "TVShowTitle": meta['tvshowtitle'], "showtitle": meta['tvshowtitle'], "tvdb": self.tvdb, "mediatype": self.content})
#                xbmc.log(str(self.infolabels)+'11111_1===>PHIL', level=xbmc.LOGINFO)
#                xbmc.log(str(self.content)+'11111_1===>PHIL', level=xbmc.LOGINFO)
            else: 
                self.infolabels.update(meta)
#                xbmc.log(str(self.infolabels)+'11111_2===>PHIL', level=xbmc.LOGINFO)
#                xbmc.log(str(self.content)+'11111_2===>PHIL', level=xbmc.LOGINFO)
#                xbmc.log(str(infoMeta)+'11111_2===>PHIL', level=xbmc.LOGINFO)
            ...
            self.infolabels.update({"plot": plot})
            #self.infolabels = {"Title": title, "Plot": plot, "year": self.year}
            item.setInfo(type='Video', infoLabels = self.infolabels)
