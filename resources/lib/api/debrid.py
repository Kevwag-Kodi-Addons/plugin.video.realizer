...
try:
	profilePath   = xbmcvfs.translatePath(addonInfo('profile')).decode('utf-8')
except:
	profilePath   = xbmcvfs.translatePath(addonInfo('profile'))
libraryPath   = xbmcvfs.translatePath(control.setting('library.path'))
manualLibrary = xbmcvfs.translatePath(control.setting('library.manual'))
...
def scrapecloud(title, year=None, season=None, episode=None):
	...
	for x in downloadList:
		try:
			...
			#xbmc.log(str(titleCheck)+'===>PHIL', level=xbmc.LOGINFO)
			#xbmc.log(str(cleantitle.get(name))+'===>PHIL', level=xbmc.LOGINFO)
	...
	# NORMAL PLAY MODE	  
		...
		for result in exactSources:
			...
			labelName = str(labelName).encode("utf-8").decode('utf-8')
			if (labelName[:2] == 'b\'' or labelName[:2] == 'b"') and (labelName[-1:] == '\'' or labelName[-1:] == '"'):
				labelName = labelName[2:-1]
			#xbmc.log(str(labelName)+'===>PHIL', level=xbmc.LOGINFO)
			label = "[B]" + str(fileLabel.upper()) + " |[/B] " + str(labelName)
			#xbmc.log(str(label)+'===>PHIL', level=xbmc.LOGINFO)
	...
	x = 0
	#xbmc.log(str(sourceNames)+'===>PHIL', level=xbmc.LOGINFO)
	sourceNames_var = {}
	for i in sourceNames:
		curr_name = str(i.decode('utf-8'))
		#if (curr_name[:2] == 'b\'' or curr_name[:2] == 'b"') and (curr_name[-1:] == '\'' or curr_name[-1:] == '"'):
		#	curr_name = curr_name[2:-1]
		sourceNames_var[x] = str(curr_name).replace('/','')
		#xbmc.log(str(sourceNames_var[x])+'===>PHIL', level=xbmc.LOGINFO)
		x = x + 1
	
	seen = {} # dict (value, key)
	result = set() # keys with unique values
	result_value = set()
	for k,v in sourceNames_var.items():
		if v in seen:
			...
			result.add(v)
			result_value.add(k)
	#xbmc.log(str(result_value)+'===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(result)+'===>PHIL', level=xbmc.LOGINFO)
	
	x = len(exactSources)-1
	len_x = len(exactSources)
	...
	#xbmc.log(str()+'labels===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(labels)+'labels===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str()+'sources===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(sources)+'sources===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str()+'types===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(types)+'types===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str()+'IDs===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(IDs)+'IDs===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str()+'sourceNames===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(sourceNames)+'sourceNames===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str()+'normalSources===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(normalSources)+'normalSources===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str()+'exactSources===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(exactSources)+'exactSources===>PHIL', level=xbmc.LOGINFO)

	old_i = ''
	#xbmc.log(str('for i in sources:')+'===>PHIL', level=xbmc.LOGINFO)
	for i in sources:
		...
		if old_i == '':
			old_i = i[52:]
			i_count = i_count +1
		if (i[52:] != old_i and old_i != '') or int_i==0:
			old_i = i[52:]
			i_count = i_count +1
	"""
	for i in sourceNames:
		if old_i == '':
			old_i = i
			i_count = i_count +1
		if (i != old_i and i != '/' + old_i) and old_i != '':
			old_i = i
			i_count = i_count +1
	x = 0
	for i in types:
...
#	i_count = 2
	...
	if i_count > 1:
		...
		try:
			selected_name = sourceNames[select].decode('utf-8')
		except:
			selected_name = sourceNames[select]
	else:
		selected_type = types[0]
		selected_url = sources[0]
		selected_id = IDs[0]
		try:
			selected_name = sourceNames[0].decode('utf-8')
		except:
			selected_name = sourceNames[0]
		
	#xbmc.log(str(selected_type)+'===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(selected_url)+'===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(selected_id)+'===>PHIL', level=xbmc.LOGINFO)
	#xbmc.log(str(selected_name)+'===>PHIL', level=xbmc.LOGINFO)
	
	if selected_type != 'download': 
		torrName = selected_name
		torrFile = torrentItemToDownload(torrName, selected_id)
		control.infoDialog('Playing Torrent Item', torrFile['filename'], time=3)
		#xbmc.log(str(torrFile['download'])+'torrFile[\'download\']===>PHIL', level=xbmc.LOGINFO)
		#xbmc.log(str(torrFile['id'])+'torrFile[\'id\']===>PHIL', level=xbmc.LOGINFO)
		return torrFile['download'], torrFile['id']
	else: 
		#xbmc.log(str(selected_url)+'selected_url===>PHIL', level=xbmc.LOGINFO)
		#xbmc.log(str(selected_id)+'selected_id===>PHIL', level=xbmc.LOGINFO)
...
class realdebrid:
	...
	def auth(self):
		...
		progressDialog = control.progressDialog
		progressDialog.create('Real Debrid', str(str(verification_url) + str(user_code) + str(line3)))
		...
		for i in range(0, int(expires_in)):
			...
			if 1==1:
				...
				progressDialog.update(int(percent), str(str(verification_url) + str(user_code) + str(line3)))
				
				time.sleep(30)
				xbmc.log(str(device_code)+'===>PHIL', level=xbmc.LOGINFO)
				credentials = self.getCredentials(device_code)
				xbmc.log(str(credentials)+'===>PHIL', level=xbmc.LOGINFO)
...
def normalize(txt):
	try:
		txt = re.sub(r'[^\x00-\x7f]',r'', txt.decode('utf-8'))
	except:
