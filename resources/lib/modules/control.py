...
def moderator():
    ...
    #xbmc.log(str(urlparse.urlparse(sys.argv[0]).netloc)+'===>REALIZER_CONTROL', level=xbmc.LOGINFO)
#    xbmc.log(str(infoLabel('Container.PluginName'))+'===>REALIZER_CONTROL', level=xbmc.LOGINFO)
#    import xbmcgui
#    xbmc.log(str(xbmcgui.getCurrentWindowDialogId())+'===>REALIZER_CONTROL', level=xbmc.LOGINFO)


    import xbmcgui
    curr_lock = xbmcgui.Window(10000).getProperty('REALIZER.ResolvedUrl')
    player = xbmc.Player()
    xbmc.log(str(netloc)+'===>PHIL', level=xbmc.LOGINFO)
    xbmc.log(str(infoLabel('Container.PluginName'))+'==REALIZER_CONTROL===>PHIL', level=xbmc.LOGINFO)
    if player.isPlaying()==1:
        xbmc.log(str('exit')+'==REALIZER_CONTROL===>PHIL', level=xbmc.LOGINFO)
        sys.exit()

    if curr_lock == 'true':
        xbmc.log(str('curr_lock_true')+'==REALIZER_CONTROL===>PHIL', level=xbmc.LOGINFO)
        xbmcgui.Window(10000).clearProperty('REALIZER.ResolvedUrl')
        return
    
    elif not infoLabel('Container.PluginName') in netloc and not 'plugin.video.themoviedb.helper' in str(infoLabel('Container.PluginName')):
        xbmc.log(str('exit')+'==REALIZER_CONTROL===>PHIL', level=xbmc.LOGINFO)
        sys.exit()
    if 'plugin.video.themoviedb.helper' in str(infoLabel('Container.PluginName')):
