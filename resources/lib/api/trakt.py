...
def authTrakt():
    try:
        ...
        progressDialog.create('Trakt', six.ensure_str( verification_url ) + six.ensure_str( user_code ) )
        ...
        result = requests.get(urljoin(BASE_URL, '/users/me'), headers=headers).content
        result = utils.json_loads_as_str(result)
