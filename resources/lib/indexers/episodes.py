...
import libThread
import os, sys, re, json, zipfile, datetime, json, time

import six
from six import BytesIO
from six.moves import urllib
from six.moves.urllib_parse import urlparse, parse_qsl, urlencode, urlsplit, quote_plus, unquote_plus
...
class seasons:
    ...
    def tvdb_list(self, tvshowtitle, year, imdb, tvdb, lang, limit='', season=None, episode=None):
        ...
        try:
            ...
            if tvdb == '0':
                url = self.tvdb2_by_query % (quote_plus(self.list[i]['title']))
...
class episodes:
    def __init__(self):
        ...
        if six.PY3:
            key = base64.b64encode(bytes(self.tvdb_key, 'utf-8'))
        else:
            key = self.tvdb_key.decode('base64')
        self.tvdb_info_link = 'http://thetvdb.com/api/%s/series/%s/all/%s.zip' % (key, '%s', '%s')
        self.tvdb_image = 'http://thetvdb.com/banners/'
        self.tvdb_poster = 'http://thetvdb.com/banners/_cache/'
    ...
    def trakt_episodes_list(self, url, user, lang):
        ...
        def items_list(i):
            ...
            try:
                ...
                zip = zipfile.ZipFile(BytesIO.BytesIO(data))
    ...
    def inProgressDir(self, items):
        ...
        for i in items:
            try:
                ...
                bookmarkname = quote_plus(i['tvshowtitle'].lower())
                bookmarkname = bookmarkname + quote_plus(' S%02dE%02d' % (int(i['season']), int(i['episode'])))                
                bookmarkname = unquote_plus(self.bookmarkname)
    ...
    def episodeDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                bookmarkname = quote_plus(i['tvshowtitle'].lower())
                bookmarkname = bookmarkname + quote_plus(' S%02dE%02d' % (int(i['season']), int(i['episode'])))                
                bookmarkname = unquote_plus(bookmarkname)
