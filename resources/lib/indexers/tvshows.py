...
from six.moves.urllib_parse import urlparse, parse_qsl, urlencode, urlsplit, quote_plus
...
class tvshows:
    ...
    def getSearch(self, title = None):
        try:
            ...
            url = '%s/search/series?name=%s'  % (self.tvdb2_api, quote_plus(query))
    ...
    def searchTvdb(self):
        try:
            ...
            url = 'https://api.thetvdb.com/search/series?name=%s'  % (quote_plus(q))
            self.getTvdb(url)
        except:
            return    
    ...
    def person(self):
        try:
            ...
            url = '%s?action=tvPersons&url=%s' % (sys.argv[0], quote_plus(url))
            self.persons(url)
        except:
            return
    ...
    def super_info(self, i):
        try:
            ...
            if imdb == '0':
                try:
                    imdb = trakt.SearchTVShow(quote_plus(self.list[i]['title']), self.list[i]['year'], full=False)[0]
