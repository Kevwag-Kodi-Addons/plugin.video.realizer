...
from resources.lib.modules import cache, utils, lib_meta
...
def meta_folder(create_directory=True, content='all'):
    ...
    for result in r:
        ...
        if control.setting('metacloud.dialog') == 'true': progressDialog.update( int( prog ), r'This process will run just for items in your cloud not already in the database...' + six.ensure_str(name) )    
        ...
        try:
            ...
            if isTv == True:
                ...
                label = "%s" % (tvshowtitle)
                systitle = quote_plus(superInfo['title'])    
            else: 
            ...
            playLink = '0'
            sysmeta = quote_plus(json.dumps(superInfo))                
            year = superInfo['year']

            
            cm = []
            if isTv == True: url = '%s?action=meta_episodes&imdb=%s&tmdb=%s&tvdb=%s' % (sysaddon, imdb, tmdb, tvdb)
            else: url = '%s?action=directPlay&url=%s&title=%s&year=%s&imdb=%s&meta=%s&id=%s&name=%s' % (sysaddon, 'resolve', systitle , year, imdb, sysmeta, id, quote_plus(OriginalName))
            ...
            superInfo.update({'trailer': '%s?action=trailer&name=%s' % (sysaddon, quote_plus(metatitle))})
            try:
...
def meta_episodes(imdb=None, tvdb=None, tmdb = None, create_directory=True):
    ...
    for result in r:
            ...
            cm = []
            url = '%s?action=directPlay&url=%s&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&meta=%s&id=%s&name=%s' % (sysaddon, 'resolve', systitle, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, sysmeta, id, quote_plus(OriginalName))
    ...
    try:
        for result in r:
            try:
                ...
                if six.ensure_str( ext ) in VALID_EXT: isPlayable = True    
...
def torrentInfo(id):
    ...
    for x in files:
        try:
            ...
            if not six.ensure_str( ext.lower() ) in VALID_EXT: raise Exception()
            
            
            label = six.ensure_str( ext.upper() ) + " | " + name
...
def torrentItemToDownload(name, id):
    ...
    try:
        ...
        if FileMatch == False:  #FALLBACK TO UNRESTRICT ALL PACK
            for link in links:
                try:
                    result_2 = realdebrid().resolve(link, full=True)
...
class realdebrid:
    ...
    def auth(self):
        ...
        progressDialog = control.progressDialog
        progressDialog.create('Real Debrid', six.ensure_str( verification_url ) + six.ensure_str( user_code ) + six.ensure_str( line3 ) )
        ...
        for i in range(0, expires_in):
            try:
                ...
                #if float(i) % int( interval ) != 0: break
                
                percent = (i * 100) / expires_in
                progressDialog.update( int(percent), six.ensure_str( verification_url ) + six.ensure_str( user_code ) + six.ensure_str( line3 ) )
                
                credentials = self.getCredentials(device_code)
                if "client_secret" in str(credentials):
    ...
    def getToken(self, refresh=False):
        ...
        if refresh: 
            try:
                ...
                if token == '' or token == None or token == '0': control.infoDialog('Check Debrid Authorization in System','Realizer is not Authorized')
            except: pass
    ...
    def torrentScrapeInfo(self, id):
        try:
            ...
            for item in files:
                ...
                if not six.ensure_str( ext.lower() ) in VALID_EXT: continue
