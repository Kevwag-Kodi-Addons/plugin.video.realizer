...
def update():
	...
	sourceList = []
	xbmc.log(str(xbmc.Player().isPlaying())+'=Realizer_IS_PLAYING===>PHIL', level=xbmc.LOGINFO)
	if len(rsslist) > 0 and xbmcgui.Window(10000).getProperty('Next_EP.Realizer_Suppress') != 'True': 
		if xbmc.Player().isPlaying() == 0:
			control.infoDialog('Checking RSS Lists...')
		else:
			xbmc.log(str('Checking RSS Lists...')+'REALIZER===>PHIL', level=xbmc.LOGINFO)
	for x in rsslist:
		u = x['rss']
		timeNow =  datetime.datetime.utcnow()
	...
	if xbmc.Player().isPlaying() == 0:
		...
		xbmc.log(str('RSS Lists check completed')+'REALIZER===>PHIL', level=xbmc.LOGINFO)
